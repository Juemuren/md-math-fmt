...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider('markdown', {
    async provideDocumentFormattingEdits(document, formatting, token) {
      ...
      const config = vscode.workspace.getConfiguration('md-math-fmt', document.uri);
      ...
      try {
        const edits = await formatMathEdits(document.getText(), {
          ...
          cwd: document.uri.scheme === 'file' ? dirname(document.uri.fsPath) : vscode.workspace.getWorkspaceFolder(document.uri)?.uri.fsPath,
        ...
        if (!token.isCancellationRequested) {
          void vscode.window.showErrorMessage(`Markdown Math Formatter: ${error instanceof Error ? error.message : String(error)}`);
        }
        return [];
      } finally {
