...
async function formatLatex(input: string, inline: boolean, options: FormatOptions): Promise<string> {
  const executable = options.texFmtPath ?? 'tex-fmt';
  const args = ['--stdin', '--quiet'];
  args.push(...(options.configPath ? ['--config', options.configPath] : ['--noconfig']));
  if (inline) args.push('--nowrap');
  else if (options.lineWidth !== undefined) args.push('--wraplen', String(options.lineWidth));
  if (options.tabSize !== undefined) args.push('--tabsize', String(options.tabSize));
  ...
  return new Promise((resolve, reject) => {
    const child = execFile(executable, args, {
      ...
      if (error) {
        reject(new Error(`Failed to run ${executable}: ${stderr.trim() || error.message}`, { cause: error }));
      } else {
...
export async function formatMathEdits(source: string, options: FormatOptions = {}): Promise<MathEdit[]> {
  ...
  const nodes: Array<{ type: string; value: string; start: number; end: number; line: number }> = [];
  visit(tree, (node) => {
    if (node.type !== 'math' && node.type !== 'inlineMath') return;
    const start = node.position?.start.offset;
    const end = node.position?.end.offset;
    if (start !== undefined && end !== undefined) {
      nodes.push({ type: node.type, value: node.value, start: start + offset, end: end + offset, line: node.position!.start.line });
    }
  });
  ...
  for (const node of nodes) {
    ...
    try {
      if (node.type === 'inlineMath') {
        ...
        if (text !== raw) edits.push({ start: node.start, end: node.end, text });
      } else {
        ...
        const text = value.length === 0 ? '' : value.split('\n').map(line => prefix + line).join(eol) + eol;
        const start = node.start + opening[0].length;
        const end = node.start + closing.index + closing[0].match(/^(\r\n|\n|\r)/)![0].length;
        if (start > end) continue;
        if (source.slice(start, end) !== text) edits.push({ start, end, text });
      }
    } catch (error) {
      throw new Error(`Math at line ${node.line}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
    }
  }
  return edits;
}

export async function formatMarkdown(source: string, options: FormatOptions = {}): Promise<string> {
