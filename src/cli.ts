...
import { formatMarkdown, type FormatOptions } from './index.js';
...
async function main(): Promise<void> {
  ...
  if (values.version) { process.stdout.write(`${metadata.version}\n`); return; }
  ...
  if (files.includes('-') && files.length > 1) throw new Error('stdin cannot be combined with other files');
  if (values.write && files.includes('-')) throw new Error('--write requires a file');
  if (files.length > 1 && !values.write && !values.check) throw new Error('Multiple files require --write or --check');
  const lineWidth = values['line-width'] === undefined ? undefined : Number(values['line-width']);
  if (lineWidth !== undefined && (!Number.isSafeInteger(lineWidth) || lineWidth < 1)) {
    throw new Error('--line-width must be a positive integer');
  }
  const texFmtPath = values['tex-fmt'];
  const options: FormatOptions = {
    texFmtPath: texFmtPath && /[\\/]/.test(texFmtPath) ? resolve(texFmtPath) : texFmtPath,
  ...
  for (const file of [...new Set(files)]) {
    let input: string;
    if (file === '-') {
      if (process.stdin.isTTY) throw new Error('Provide a Markdown file or pipe input on stdin');
    ...
    const output = await formatMarkdown(input, { ...options, cwd: file === '-' ? process.cwd() : dirname(resolve(file)) });
...
main().catch(error => {
  process.stderr.write(`md-math-fmt: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 2;
});
