...
let available = true;
try {
  execFileSync('tex-fmt', ['--version'], { stdio: 'ignore' });
} catch {
  available = false;
}
const integration = {
...
test('preserves ordinary Markdown and code without invoking tex-fmt', async () => {
  const source =
    '# 标题\r\n\r\n`$ x $` and \\$5\r\n\r\n```math\r\n$$\r\nx\r\n$$\r\n```\r\n\r\n    $ code $\r\n';
  assert.equal(
    await formatMarkdown(source, { texFmtPath: 'missing-formatter' }),
    source,
  );
});

test(
  'formats real block math and leaves surrounding text byte-for-byte intact',
  integration,
  async () => {
    const input = `\uFEFF# 数学 🧮  \n\n${formula}\n\n[link](./file.md)  \n`;
...
test(
  'formats inline math with preserved delimiters and padding where required',
  integration,
  async () => {
    const input = 'a $ x $ b $$ y $$ c $\\omega_X=\\star X^\\flat$';
    assert.equal(
      await formatMarkdown(input),
      'a $x$ b $$y$$ c $\\omega_X=\\star X^\\flat$',
    );
    const dollar = 'a $$ $x$ $$ b';
...
test('preserves multiline inline math and unclosed block math', async () => {
  for (const input of [
    ...
    assert.equal(
      await formatMarkdown(input, { texFmtPath: 'missing-formatter' }),
      input,
    );
  }
});
...
test(
  'preserves CRLF and formats formulas in nested Markdown containers',
  integration,
  async () => {
...
test(
  'preserves longer fences and metadata, and handles empty blocks',
  integration,
  async () => {
    const input = formula
      .replace(/^\$\$/, () => '$$$$ label')
...
test(
  'accepts an explicit tex-fmt configuration path containing spaces',
  integration,
  async () => {
    ...
    try {
      ...
      assert.equal(
        await formatMarkdown(formula, { configPath }),
        formatted.replace(/^ {2}/gm, '    '),
      );
    } finally {
...
test('reports process failures with formula locations', async () => {
  await assert.rejects(
    formatMarkdown('text\n\n$x$', { texFmtPath: 'missing-formatter' }),
    /Math at line 3: Failed to run/,
  );
});

test('validates options and respects cancellation', async () => {
  await assert.rejects(
    formatMarkdown('', { lineWidth: 0 }),
    /positive integer/,
  );
  await assert.rejects(
    formatMarkdown('', { tabSize: 1.5 }),
    /positive integer/,
  );
  const controller = new AbortController();
  controller.abort();
  await assert.rejects(
    formatMarkdown('$x$', { signal: controller.signal }),
    /abort/i,
  );
});
