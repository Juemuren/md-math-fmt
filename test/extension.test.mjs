...
function loadExtension({ trusted = true, executable = 'tex-fmt' } = {}) {
  ...
  const vscode = {
    languages: {
      registerDocumentFormattingEditProvider(language, value) {
    ...
    workspace: {
      ...
      getConfiguration() {
        return {
          get(key, fallback) {
            return key === 'texFmtPath' ? executable : fallback;
          },
        };
      },
    },
...
test('extension registers a provider and returns formula-only edits', {
  skip:
    spawnSync('tex-fmt', ['--version']).status !== 0 &&
    'Install tex-fmt to run integration tests',
}, async () => {
  const extension = loadExtension();
  const edits = await extension.provider.provideDocumentFormattingEdits(
    document(),
    formatting,
    extension.token,
  );
...
test('extension reports formatter failures without editing the document', async () => {
  ...
  assert.deepEqual(
    await extension.provider.provideDocumentFormattingEdits(
      document(),
      formatting,
      extension.token,
    ),
...
test('extension skips untrusted workspaces and cancelled requests', async () => {
  const untrusted = loadExtension({
    trusted: false,
    executable: 'missing-formatter',
  });
  assert.deepEqual(
    await untrusted.provider.provideDocumentFormattingEdits(
      document(),
      formatting,
      untrusted.token,
    ),
  ...
  assert.deepEqual(
    await cancelled.provider.provideDocumentFormattingEdits(
      document(),
      formatting,
      cancelled.token,
    ),
